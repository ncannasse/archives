	...
	*/
	...
	function encode_string(s : String) {
		...
		for(i=0;i<s._length;i++) {
	 	...
	 	encode_int(s._length);
		if( is_b64 ) {
			bc.write(1,0);
			var bc = bc;
			cpos.iter(fun(p) { bc.write(6,p) });
 		} else {
			bc.write(2,is_ascii?2:3);
			for(i=0;i<s._length;i++)
				bc.write(is_ascii?7:8,s.at(i));
		}
	}
	...
	function decode_string() {
		...
		var s = Std.cast(String).new(Std.neko("$smake(len)"));
		var i;
		if( is_b64 ) {
			for(i=0;i<len;i++) {
				var c = bc.read(6);
				Std.neko("$sset(s.@s,i,$sget(BitCodec.ENCODE.@s,c))");
			}
		} else {
			var is_ascii = (bc.read(1) == 0);
			for(i=0;i<len;i++) {
				var c = bc.read(is_ascii?7:8);
				Std.neko("$sset(s.@s,i,c)");
			}
		}
		return Std.cast(s);
	}
	...
	function encode_object(o) {
		...
		var me = this;
		f.iter(fun(fid) {
			me.encode_object_field(Std.fieldName(fid),Std.field(o,fid))
		});
		bc.write(2,3);
	}

	function encode_object_field(k : String,d) {
		if( Std.typeof(d) != Std.types.tfunction && d != null ) {
			if( obfu_mode && k.charAt(0) == "$" )
				k = k.sub(0,k._length-1);
	...
	function decode_object() {
		...
		while( true ) {
			...
			Std.setField(o,Std.fieldId(k),do_decode());
		}
		return Std.cast(o);
	}
	...
	function decode_float() {
		var l = bc.read(5);
		var i;
		var s : String = Std.cast(String).new(Std.neko("$smake(l)"));
		for(i=0;i<l;i++) {
			var k = bc.read(4);
			if( k < 10 )
				k += 48;
			...
			Std.neko("$sset(s.@s,i,k)");
		}
		return Std.cast(Std.parseFloat(s));
	}

	function do_encode(o : 'a) : bool {
		switch( Std.typeof(o) ) {
		case Std.types.tnull:
			bc.write(4,15);
			break;
		case Std.types.tint:
			bc.write(2,0);
			encode_int(Std.cast(o));
			break;
		case Std.types.tfloat:
			var n = Std.cast(o);
			if( n == 0 * null )
				bc.write(4,6);
			else if( n == 1 / 0 )
			...
			break;
		case Std.types.tbool:
			if( Std.cast(o) == true )
				bc.write(4,13);
			else
				bc.write(4,12);
			break;
		case Std.types.tobject:
			if( Std.typeof(Std.neko("o.@a")) == Std.types.tarray ) {
				bc.write(3,4);
				encode_array(Std.cast(o));
			} else if( Std.typeof(Std.neko("o.@s")) == Std.types.tstring ) {
				bc.write(4,14);
			...
			break;
		default:
			Std.throw("Unsupported type#"+Std.typeof(o));
			break;
		}
		return true;
	}
	...
	function encode( o ) {
 	...
 	function decode( data ) {
		...
		if( crc ) {
			...
			var s2;
			try {
				s2 = data.sub(bc.in_pos,4);
			} catch( e ) {
				s2 = null;
			}
			if( s1 != s2 )
				Std.throw("Invalid CRC");
		}
		return o;
	}
