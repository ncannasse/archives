...
__inline void avlrotleft(objtable *n) {
	objtable tmp = *n;
	*n = (*n)->right;
	tmp->right = (*n)->left;
	(*n)->left = tmp;
...
__inline void avlrotright(objtable *n) {
	objtable tmp = *n;
	*n = (*n)->left;
	tmp->left = (*n)->right;
	(*n)->right = tmp;
...
__inline unsigned int avlleftgrown(objtable *n) {
	switch( (*n)->skew ) {
	case BAL_LEFT:
...
__inline unsigned int avlrightgrown(objtable *n) {
	switch( (*n)->skew ) {
	case BAL_LEFT:					
		(*n)->skew = BAL_NONE;
		return OK;
...
unsigned int _otable_replace(objtable *n, field id, value data) {
	unsigned int tmp;
	if( !(*n) ) {
		*n = (objtable)alloc_small(sizeof(struct _objtable));
		(*n)->left = NULL;
		(*n)->right = NULL;
		(*n)->id = id;
		(*n)->data = data;
...
__inline unsigned int avlleftshrunk(objtable *n) {
	switch( (*n)->skew ) {
	case BAL_LEFT:
		(*n)->skew = BAL_NONE;
		return BALANCE;
...
__inline unsigned int avlrightshrunk(objtable *n) {
	switch( (*n)->skew ) {
	case BAL_RIGHT:
		(*n)->skew = BAL_NONE;
		return BALANCE;
...
__inline int avlfindhighest(objtable target, objtable *n, unsigned int *res)  {
	objtable tmp;
	*res = BALANCE;
	if( !(*n) )
		return 0;
...
__inline int avlfindlowest(objtable target, objtable *n, unsigned int *res) {
	objtable tmp;
	*res = BALANCE;
	if( !(*n) )
		return 0;	
...
objtable otable_copy(objtable t) {
	...
	if( t == NULL )
		return NULL;
	nt = (objtable)alloc_small(sizeof(struct _objtable));
	nt->id = t->id;
	nt->data = t->data;
	nt->skew = t->skew;
...
objtable otable_empty() {
	objtable t2 = (objtable)alloc_small(sizeof(struct _objtable));
	t2->count = 0;
	t2->cells = NULL;
	return t2;
}
...
void otable_replace( objtable t, field id, value data ) {
	...
	mid = (min + max) >> 1;
	c = (cell*)alloc_small(sizeof(cell)*(t->count + 1));
	min = 0;
	while( min < mid ) {
		c[min] = t->cells[min];
		min++;
...
objtable otable_copy( objtable t ) {
	objtable t2 = (objtable)alloc_small(sizeof(struct _objtable));
	t2->count = t->count;
	t2->cells = (cell*)alloc_small(sizeof(cell)*t->count);
