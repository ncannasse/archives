...
template <class KEY,class ITEM> class WMap {
...
public:
	...
	typedef  int	(*ANYCOMPARATOR)( KEY, ITEM, void *);// comparative function
...
private:
	...
	// find the specified item matched by the specified compatator %CN%
	Cel<KEY,ITEM> *FindAny( Cel<KEY,ITEM> *c, ANYCOMPARATOR comp, void *param ) {		
		if( c == NULL )
			return NULL;
		if( comp(c->key,c->item,param) == 0 )
			return c;
		Cel<KEY,ITEM> *r;
